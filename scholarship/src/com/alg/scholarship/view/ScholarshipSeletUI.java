...
public class ScholarshipSeletUI extends IndexAdmin {

	public ScholarshipSeletUI(String username) {
		super(username);
		init();
				...
				if (scholarshipid == 0) {
					...
					if (mess == 0) {
						...
						int del = scholarshipDao
								.delScholarshipById(scholarshipid);
						if (del == 1) {
							...
							// 数据
							Vector list = scholarshipDao.findAllScholarship();
		...
		updateButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (scholarshipid == 0) {
					...
					index.setVisible(false);
					new ScholarshipUpdateUI(username, scholarshipid);
				}
			}
		});
	}
